...
LIBECS_DM_CLASS( Circulation1Process, Process )
{

 public:
  
  LIBECS_DM_OBJECT( Circulation1Process, Process ){
  ...
  virtual void fire(){
    ...
    int i = getStepper()->getCurrentTime() /5.5;
    theCalcTime = getStepper()->getCurrentTime() - 7.5 * i;
    
    if (getStepper()->getCurrentTime() > 5.5){
...
LIBECS_DM_INIT( Circulation1Process, Process );
